...
const ELEMENT_DATA: PeriodicElement[] = [
  ...
  // {ContractNo:"5003253358",od_amt:"1",od_num:"2",original_cust_name:"nikhil",vehicle_model:"tata",vehicle_reg_no:"1212132"},  {ContractNo:"5003253360",od_amt:"1",od_num:"2",original_cust_name:"nikhil",vehicle_model:"tata",vehicle_reg_no:"1212132"},
...
feedbackData: string="";
  ...
  constructor(private router: Router,private formBuilder: FormBuilder, private http: HttpClient, private _snackBar: MatSnackBar,
     private shared : RequestInitationService,private datePipe: DatePipe) {
  ...
  RequestInit() {
   ...
   // else if(this.RequestForm.value.Bp_name==="" || this.RequestForm.value.Contract_no==="" || this.RequestForm.value.Group_code===""){
    ...
    const prDateValue = this.RequestForm.value.Pr_date;
    const formattedPrDate = this.datePipe.transform(prDateValue, 'dd-MM-yyyy');
    console.log('Formatted Pr_date:', formattedPrDate);

      this.isShowSpin = !this.isShowSpin;
      console.log("Request initiation form data", this.RequestForm.value);
      this.RequestForm.value.Pr_date=formattedPrDate;
...
//OMSWACHATA
  ...
  abc() {
    ...
    console.log("api request1" + JSON.stringify(Apirequest1));
    ...
    const body = { title: 'Angular Post', content: 'Sending data to API' };
     ...
     timer(2000).subscribe(i=>{this.router.navigate(['/FeedbackForm']);})
